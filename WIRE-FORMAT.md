...
broadcast plane. Implemented by this project (`daemon.c` for the joint format, `transmitter.c`
and `receiver.c` for the legacy one) and by
[Mercury](https://github.com/Rhizomatica/mercury)
(`datalink_broadcast/bcast_file.c`); this document is kept identical in both
repositories.
...
All rows were verified by running the real binaries at each end of two Mercury
modems over a simulated HF path (Mercury's `utils/bcast_ota_test.sh`).
